...
*/
	...
	function easyreservations_generate_multical($date, $monthes, $day = 0){
		$timenows = '';
		for($i = 0; $i < $monthes; $i++){
			if($day == 0) $timenows[] =  mktime(0, 0, 0, date("m", time())+$date+$i, 1, date("Y", time()));
			else $timenows[] = strtotime(date("d.m.Y", time()))+($date*86400)+($i*$day*86400);
		}
		return $timenows;
	}
