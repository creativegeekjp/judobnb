						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									<?php $array = array('US Dollar' => 'usd', 'Canadian Dollar' => 'cad'); 
										foreach($array as $key => $currency){
											if($ccoptions['stripe']['currency'] == $currency) $sel = 'selected="selected"'; else $sel = '';
...
JAVASCRIPT;
				...
				$data = ''; $label = __('Pay', 'easyReservations'); $sublabel = __('Secure', 'easyReservations');
				if($ccoptions['stripe']['address'] == 1) $data = "data-address";
				...
				$ccform='<script src="https://checkout.stripe.com/v2/checkout.js" class="stripe-button" data-name="'.$label.'" data-description="'.$sublabel.'" data-key="'.$stripekey.'" data-amount="'.str_replace(array(',','.'), '',  number_format($price,2,'.','.')).'" '.$data.'></script>';
				$ccform .=<<<JAVASCRIPT
<script type="text/javascript">
var readyStateCheckIntervalTwo = setInterval(function() {
  if(document.readyState === "complete"){
		jQuery('.stripe-button').bind('token', function(e, token){
			//console.log('Your token is:', token.id);
			var data = {
...
JAVASCRIPT;
	...
	function easyreservations_get_credit_cards_options($sel = false){
		...
		foreach($cards as $card){
			if($sel == $card) $selected = 'selected="selected"'; else $selected = '';
	...
	function easyreservations_credit_card_callback(){
		...
		if(isset($_POST['token'])){
			include_once(WP_PLUGIN_DIR."/easyreservations/lib/modules/paypal/stripe/Stripe.php");
			try {
				Stripe::setApiKey($ccoptions['stripe']['seckey']);
				$token = $_POST['token'];
				...
				$pricecent = (int) str_replace(array(',','.'), '', number_format($price,2,'.','.'));

				if($ccoptions['stripe']['desc'] == 'email') $desc = $res->email;
				else $desc = $res->name;
				// create the charge on Stripe's servers - this will charge the user's card
				$charge = Stripe_Charge::create(array(
					"amount" => $pricecent, // amount in cents, again
		...
		if($check === true){
			foreach(explode('-', $_POST['id']) as $id){
				...
				try {
					$edit = '';
					if($ccoptions['approve'] == 1 && $res->status != 'yes'){
						$edit[] = 'status';
						$edit[] = 'resourcenumber';
