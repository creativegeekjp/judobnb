...
if(is_admin()){
	...
	function easyreservations_add_userpc_settings_tab(){
		if(isset($_GET['site']) && $_GET['site'] == "userpc") $current = 'current'; else $current = '';
	...
	function easyreservations_save_usercp_settings(){
		if(isset($_POST['easy-chat-modus'])){
			if(isset($_POST['easy-chat-modus'])) $modus = $_POST['easy-chat-modus']; else $modus = 0;
			if(isset($_POST['easy-chat-img'])) $img = 1; else $img = 0;
			if(isset($_POST['easy-chat-time'])) $time = 1; else $time = 0;
			if(isset($_POST['easy-chat-dummy'])) $dummy = 1; else $dummy = 0;
			if(isset($_POST['easy-chat-title'])) $title = $_POST['easy-chat-title']; else $title = '';
			if(isset($_POST['easy-chat-dummy-user'])) $dummy_user = $_POST['easy-chat-dummy-user']; else $dummy_user = 0;
			if(isset($_POST['easy-chat-dummy-message'])) $dummy_message = $_POST['easy-chat-dummy-message']; else $dummy_message = 0;
			if(isset($_POST['easy-chat-timetodelete'])) $timetodelete = $_POST['easy-chat-timetodelete']; else $timetodelete = 0;
			if(isset($_POST['easy-chat-guest-img'])) $guest_img = $_POST['easy-chat-guest-img']; else $guest_img = 0;
			if(isset($_POST['easy-chat-table'])) $table = 1; else $table = 0;
		...
		if(!empty($chat_options) && is_array($chat_options)) $options = $chat_options;
			...
			<table class="<?php echo RESERVATIONS_STYLE; ?>" style="width:99%;margin-top:7px">
				...
				<tbody>
					...
					<tr>
						<td style="vertical-align:top;font-weight:bold"><?php echo __( 'Informations' , 'easyReservations' );?></td>
						<td>
					...
					<tr class="alternate">
						...
						<td>
							<select name="easy-chat-timetodelete">
								...
								<option value="10800" <?php echo selected($options['timetodelete'], 10800); ?>>3 <?php echo __( 'Hours' , 'easyReservations' );?></option>
								<option value="43200" <?php echo selected($options['timetodelete'], 43200); ?>>12 <?php echo __( 'Hours' , 'easyReservations' );?></option>
								<option value="86400" <?php echo selected($options['timetodelete'], 86400); ?>>1 <?php echo __( 'Day' , 'easyReservations' );?></option>
								<option value="604800" <?php echo selected($options['timetodelete'], 604800); ?>>1 <?php echo __( 'Week' , 'easyReservations' );?></option>
								<option value="2419200" <?php echo selected($options['timetodelete'], 2419200); ?>>1 <?php echo __( 'Month' , 'easyReservations' );?></option>
							</select>
						</td>
					</tr>
				</tbody>
	...
	function easyreservations_edit_shortcode($atts){
		...
		if(isset($theMail) && !empty($theMail) && isset($theID) && is_numeric($theID) && $theID > 0){
			...
			if($show_table == 1){
				...
				$other_query = $wpdb->get_results( $wpdb->prepare( "SELECT id, name, arrival, departure, number, childs, room, roomnumber, approve, reservated, customp FROM ".$wpdb->prefix ."reservations WHERE email='%s' $status_sql $time_sql ORDER BY arrival DESC", array( $theMail )));
			...
			try {
				...
				$return .= $paypal_overlay.'<div class="easy-edit-status">';
					if(!$invoice) $class='current'; else $class = '';
					$return .= '<a href="'.$the_link.'?edit" class="'.$class.'">'.__( 'Informations' , 'easyReservations' ).'</a>';
