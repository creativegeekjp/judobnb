...
function geodir_insert_term_count_by_loc($location_name, $location_type, $loc, $count_type, $row_id=null) {
    global $wpdb, $gd_update_terms, $gd_term_post_type;
    ...
    if ( $row_id ) {
		...
		switch($location_type) {
			case 'gd_country':
				$fields .= ", country_slug='" . $location_name . "'";
			break;
			case 'gd_region':
				$fields .= ", region_slug='" . $location_name . "'";
				$fields .= ", country_slug='" . $gd_country . "'";
			break;
			case 'gd_city':
			case 'gd_neighbourhood':
				$fields .= ", region_slug='" . $gd_region . "'";
				$fields .= ", country_slug='" . $gd_country . "'";
			break;
		}
...
function geodir_get_loc_term_count($count_type = 'term_count', $location_name=null, $location_type=null, $loc=false, $force_update=false ) {
    ...
    if ($location_name && $location_type) {
		...
		switch($location_type) {
			case 'gd_country':
				$where .= " AND country_slug='" . $location_name . "'";
			break;
			case 'gd_region':
				$where .= " AND region_slug='" . $location_name . "'";
				$where .= " AND country_slug='" . $gd_country . "'";
			break;
			case 'gd_city':
			case 'gd_neighbourhood':
				$where .= " AND region_slug='" . $gd_region . "'";
				$where .= " AND country_slug='" . $gd_country . "'";
			break;
		}
		
		$sql = $wpdb->prepare( "SELECT * FROM " . GEODIR_TERM_META . " WHERE location_type=%s AND location_name=%s " . $where . " LIMIT 1", array( $location_type, $location_name ) );
