	...
	<table id="geodir_location_seo_settings" style="width:100%" cellpadding="5" class="widefat post fixed gd-seo-tbltype-<?php echo $location_type;?>">
		<thead>
			<tr>
				...
				<?php } ?>
				<th align="left" class="gd-seo-metath"><strong><?php _e('Meta Description', 'geodirlocation');?></strong></th>
		...
		foreach ($items as $item) {
			...
			if (!empty($seo)) {
				$meta_desc = $seo->seo_title != '' ? $seo->seo_title : $meta_desc;
			...
			if (!empty($city_meta)) {
				$meta_desc = $city_meta->city_meta;
				$location_desc = $city_meta->city_desc;
			}
		?>
		<tr class="geodir_set_location_seo">
			...
			<?php } ?>
			<td><textarea name="gd_seo[<?php echo $c;?>][meta_desc]" class="geodir_meta_keyword" placeholder="<?php esc_attr_e($meta_desc_default);?>"><?php echo stripslashes_deep($meta_desc); ?></textarea></td>
			<td><textarea name="gd_seo[<?php echo $c;?>][loc_desc]" class="geodir_meta_description" placeholder="<?php esc_attr_e($location_desc_default);?>"><?php echo stripslashes_deep($location_desc); ?></textarea></td>
			<td><?php
				if ($imageID) {
...
//					echo '<br/>';
				...
				<br/>
				<textarea name="gd_seo[<?php echo $c;?>][loc_tagline]" class="geodir_meta_description" placeholder="<?php esc_attr_e($loc_tagline_default);?>"><?php echo stripslashes_deep($location_image_tagline); ?></textarea>
			</td>
		</tr>
		<?php $c++; } } ?>
		</thead>
...
<p class="submit"><input name="save" class="button-primary" type="submit" value="<?php _e( 'Save changes', 'geodirlocation' ); ?>" /><input type="hidden" name="location_ajax_action" value="geodir_set_location_seo"></p>