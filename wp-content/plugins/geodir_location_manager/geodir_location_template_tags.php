			...
			if ($neighbourhood_active && $gd_session->get('gd_neighbourhood')) {
                if (isset($geodir_location_names['neighbourhood']) && $geodir_location_names['neighbourhood']) {
					...
					if (!empty($neighbourhood)) {
						$location = $neighbourhood->hood_name;
					}
		...
		if ($gd_what_lower = $gd_session->get('gd_' . $what_lower)) {
            ...
            if (isset($geodir_location_names[$what_lower]) && $geodir_location_names[$what_lower]) {
                ...
                if ($what_lower == 'neighbourhood') {
					...
					if (!empty($neighbourhood)) {
						$geodir_location_names['neighbourhood'] = $neighbourhood->hood_name;
					}
				} else {
...
?>
	<div class="geodir_locListing_main">
    	<div class="geodir-common geodir_loc_clearfix gdlm-loclist-cols-<?php echo $cols;?>">
			<div class="geodir-locListing_column<?php if($l_p_config['country_column']) { echo ' gdlm-loclist-col-' . $col; $col++; };?>" style="display:<?php echo ($l_p_config['country_column'] ? '' : 'none');?>;" >                        
		  ...
		  <div class="geodir-locListing_column<?php if($l_p_config['country_column']) { echo ' gdlm-loclist-col-' . $col; $col++; };?>" style="display:<?php echo ($l_p_config['region_column'] ? '' : 'none');?>;">
		  ...
		  <div class="geodir-locListing_column<?php if($l_p_config['country_column']) { echo ' gdlm-loclist-col-' . $col; $col++; };?> <?php if (!$neighbourhood_active) { ?>geodir-locListing_column_last<?php } ?>" style="display:<?php echo ($l_p_config['city_column']  ? '' : 'none');?>;">
			...
			<div class="geodir-locListing_column<?php if($l_p_config['country_column']) { echo ' gdlm-loclist-col-' . $col; $col++; };?> geodir-locListing_column_last" style="<?php echo (!$l_p_config['neighbourhood_column'] ? 'display:none;' : '')?>">
			<h2><?php _e('Neighbourhood' , 'geodirlocation');?></h2>
			<input name="loc_pick_neighbourhood_filter" type="text" style="display:<?php echo (!$l_p_config['neighbourhood_text_filter'] ? 'display:none;' : '')?>" />
			<ul class="geodir_neighbourhood_column">
				...
				$neighbourhoods = geodir_get_location_array($neighbourhood_args);
				
				if (!empty($neighbourhoods)) {
					foreach($neighbourhoods as $neighbourhood) {
				?>
				<li class="geodir_loc_clearfix">
					<a href="<?php echo geodir_location_permalink_url($base_location_link . $neighbourhood->location_link );?>"><?php echo __($neighbourhood->neighbourhood, 'geodirectory');?></a>
				</li>
				<?php
					} // end of foreach
				}//end of if
...
function geodir_location_tab_switcher($args = null) {
	...
	if ($enable_country != 'default' || $enable_region != 'default' || $enable_city != 'default') {
		$neighbourhood_active = get_option('location_neighbourhoods');
