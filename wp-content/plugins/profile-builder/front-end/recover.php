	...
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'recover_password' && wp_verify_nonce($_POST['password_recovery_nonce_field'],'verify_true_password_recovery') ) {
        ...
        if( $messageNo == '1' ) {
            ...
            if( !empty( $query[0] ) ){
                ...
                $recoveruserMailMessage1  = apply_filters( 'wppb_recover_password_message_content_sent_to_user1', _e($recoveruserMailMessage1,'profile-builder'), $requestedUserID, $requestedUserLogin, $requestedUserEmail );
