...
jQuery(document).ready(function() {
	...
	// Hide some geodirectory controls from admin
	jQuery('#geodir_search_near_addition').parent().parent().hide();
	// Home
	jQuery('#sub_home_page_settings tr:has(#geodir_set_as_home, #geodir_show_home_top_section, #geodir_show_home_right_section, #geodir_width_home_right_section, #geodir_show_home_contant_section, #geodir_width_home_contant_section, #geodir_show_home_left_section, #geodir_width_home_left_section, #geodir_show_home_bottom_section)').hide();
	jQuery('.gd-tab-content h3:contains("Home Top Section Settings")').hide();
	jQuery('.gd-tab-content h3:contains("Detail Page Tab Settings")').hide();
	jQuery('.gd-tab-content h3:contains("Related Post Settings")').hide();
	jQuery('.gd-tab-content h3:contains("Linked Events Settings")').hide();
	jQuery('.gd-tab-content h3:contains("User Dashboard Post Type Navigation Settings")').hide();
	jQuery('.gd-tab-content h3:contains("Home Map Settings")').hide();
	jQuery('.gd-tab-head dd a:contains("Sorting Options")').hide();
	jQuery('.gd-tab-content h3:contains("Near Autocompleter Settings")').hide();

	jQuery('#sub_geodir_listing_settings tr:has(#geodir_show_listing_top_section, #geodir_show_listing_right_section, #geodir_width_listing_right_section, #geodir_listing_view, #geodir_width_listing_contant_section, #geodir_show_listing_left_section, #geodir_width_listing_left_section, #geodir_show_listing_bottom_section, #geodir_desc_word_limit, #geodir_listing_new_days)').hide();
	jQuery('#geodir_search_settings').hide();
	jQuery('#sub_geodir_detail_settings tr:has(#geodir_show_detail_top_section, #geodir_show_detail_bottom_section, #geodir_detail_sidebar_left_section, #geodir_disable_gb_modal, #geodir_disable_user_links_section, #geodir_disable_rating_info_section, #geodir_disable_listing_info_section, #geodir_detail_page_tabs_excluded, #geodir_add_related_listing_posttypes, #geodir_related_post_relate_to, #geodir_related_post_listing_view, #geodir_related_post_sortby, #geodir_related_post_count, #geodir_related_post_excerpt, #geodir_event_linked_event_type, #geodir_event_linked_sortby, #geodir_event_linked_count, #geodir_event_linked_listing_view, #geodir_event_linked_post_excerpt, #geodir_disable_google_analytics_section, #geodir_related_post_location_filter)').hide();
	jQuery('#sub_geodir_author_settings tr:has(#geodir_show_author_top_section, #geodir_show_author_right_section, #geodir_width_author_right_section, #geodir_author_view, #geodir_width_author_contant_section, #geodir_show_author_left_section, #geodir_width_author_left_section, #geodir_show_author_bottom_section)').hide();
	jQuery('#sub_geodir_navigation_settings tr:has(#geodir_add_posttype_in_main_nav, #geodir_add_posttype_in_listing_nav, #geodir_add_listing_link_add_listing_nav, #geodir_add_listing_link_user_dashboard, #geodir_favorite_link_user_dashboard, #geodir_listing_link_user_dashboard)').hide();
	jQuery('#sub_geodir_map_settings tr:has(#geodir_home_map_untick)').hide();
	jQuery('.inner_content_tab_main tr:has(#geodir_event_defalt_filter)').hide();
	jQuery('.gd-content-heading tr:has(#geodir_claim_show_author_link)').hide();
	jQuery('.inner_content_tab_main tr:has(#geodir_enable_autocompleter_near, #geodir_autocompleter_autosubmit_near, #geodir_near_me_dist)').hide();
	jQuery('.inner_content_tab_main tr:has(#geodir_reviewrating_hide_rating_summary, #geodir_reviewrating_optional_multirating)').hide();
});
