...
<div class="updated vhman-message quick-setup">
	...
	<!-- Step 1 -->
		...
		Installs Page Builder, Sky functionality, etc.
		</span></span><br />
...
function pvt_wordpress_content_importer() {
	...
	if ( is_admin() && isset($_GET['importcontent']) ) {
		...
		if ( class_exists( 'WP_Importer' ) && class_exists( 'pvt_WP_Import' ) ) {
			...
			// Start Import
			if ( !get_option('geodir_default_location') ) {
				update_option('geodir_default_location', json_decode('{"location_id":0,"country":"United States","region":"Pennsylvania","city":"Line Lexington","country_slug":"united-states","region_slug":"pennsylvania","city_slug":"line-lexington","city_latitude":"40.2883625","city_longitude":"-75.25039099999998","is_default":"1"}'));
			}
		...
		if ( $_GET['importcontent'] == 'alldemocontent' ) {
			...
			// 5: Update theme options
			update_option('vh_theme_version', 'SkyVacation');
			$countries = array('pu_textbox' => '{"country":"Latvia:187"},{"country":"Italy:188"},{"country":"France:189"},{"country":"Spain:186"},{"country":"United States:185"},{"country":"Australia:182"},{"country":"India:181"},{"country":"Malaysia:180"},{"country":"Russia:179"},{"country":"United Kingdom:178"},{"country":"Germany:167"},{"country":"Canada:165"},');
			update_option('vh_countries_options', $countries);
			update_option('vh_footer_columns', '2');

			// 6: Update listing review data
			vh_update_geodir_reviews();

			// 7: Update few geodirectory setting
			update_option('geodir_show_addlisting_nav', '0');
			update_option('geodir_show_listing_nav', '0');
