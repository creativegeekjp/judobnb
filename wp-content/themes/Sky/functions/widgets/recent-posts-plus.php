...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        <div class="news-item">
            ...
            <div class="post_container">
                ...
                {EXCERPT}
                <div class="post-info">
                    <span class="post-categories icon-folder-open">{CATEGORIES}</span>
                    <span class="post-comments icon-comment-1">{COMMENT_COUNT}</span>
                </div>
    ...
    function __construct() {
        ...
        parent::__construct('recent-posts-plus', __('Sky - Recent Posts Plus', 'vh'), $widget_ops);
    }
    ...
    function widget( $args, $instance ) {
        ...
        $new_title .= '<span>';
        for ($i=2; $i < count(explode(' ', $title)); $i++) {
            $exploded_title = explode(' ', $title);
            $new_title .= $exploded_title[$i] . ' ';
        }
        $new_title .= '</span>';

        echo $before_title . $new_title . $after_title;
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if($include_post_thumbnail == "false" || get_the_post_thumbnail($ID, 'gallery-medium') == '' ) {
                    ...
                    if ( $post_thumbnail_width ==  '' || $post_thumbnail_height == '' ) {
                        $POST_THUMBNAIL = get_the_post_thumbnail($ID, 'popular-destinations-square');
                    } else {
