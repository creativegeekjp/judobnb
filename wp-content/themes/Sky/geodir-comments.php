...
<div id="comments">
	...
	<?php
		...
		if ( geodir_is_page('preview') ) {
			...
			?> <p class="preview"><?php _e( 'This is a preview, there\'s no reviews for this listing.', 'vh' ); ?></p></div><!-- end of comments --> <?php
	...
	<div class="btn-translate" onClick="translateTxt()">translate</div>
			<?php
				...
				if ( in_array(get_post_type(), $geodir_post_types) ) {
					...
					printf( _n( '<span class="post_comment_title">One review</span> %2$s %3$s', '<span class="post_comment_title">%1$s reviews</span> %2$s %3$s', get_comments_number(),'vh' ),
					number_format_i18n( get_comments_number() ), get_listing_rating_stars( get_listing_rating_count( $post->ID, true, $post_type ) ), do_action('vh_comment_sorting') );
					// echo '</span>';
				} else {
		...
		<ul class="commentlist">
			<?php
				...
				wp_list_comments( array( 'callback' => 'vh_geodir_comment' ) );
			?>
		</ul>
	...
	<?php if ( in_array(get_post_type(), $geodir_post_types) ) { ?>
		<div class="add-review-container"><span><?php _e("Have you been here?", "vh"); ?></span>
			...
			?>
			<b href="#" class="wpb_button wpb_btn-primary wpb_btn-small simplemodal-login">
			<?php _e('Add review', 'vh'); ?></b>
			<input id="listing-name" type="hidden" value="<?php echo get_the_title(); ?>">
			<?php
			} else {
			?>
			<a href="#" class="wpb_button wpb_btn-primary wpb_btn-small">
			<?php _e('Add review', 'vh'); ?></a>
			<input id="listing-name" type="hidden" value="<?php echo get_the_title(); ?>">
			<?php
			}
