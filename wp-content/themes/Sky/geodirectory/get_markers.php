...
if( isset( $_REQUEST['ajax_action'] ) && $_REQUEST['ajax_action'] == 'cat' ){
	...
	if(isset($_REQUEST['post_preview']) && $_REQUEST['post_preview'] != '' && isset($_SESSION['listing'])){
		...
		echo vh_geodir_get_infowindow_html($post, $_REQUEST['post_preview']);
		...
		if($postinfo)
		{
			...
			{
				echo vh_geodir_get_infowindow_html($postinfo_obj);
			}
		}
...
function get_markers(){	
	...
	if(isset($_REQUEST['search']) && !empty($_REQUEST['search']) && $_REQUEST['search']!=__( 'Title', 'vh' )){
		...
		{ 	
		 ...
		 if($default_cat != ''){
			...
			if(!empty($geodir_cat_icons) && is_array($geodir_cat_icons) && array_key_exists($default_cat,$geodir_cat_icons)){
					...
					$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($catinfo_obj->post_id), "gallery-medium" );
					$icon = $thumb["0"];	
